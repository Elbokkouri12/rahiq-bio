...
// Old Shopify product slug → new slug mapping
const SHOPIFY_PRODUCT_MAP = {
  'عسل-الدغموس': 'daghmous',
  'عسل-الدغموس-rahiq-bio': 'daghmous',
  'أملو-كاوكاو-rahiq-bio-1': 'amlou-cacao',
  'amlou-cacao-rahiq-bio-1': 'amlou-cacao',
  'عسل-السدر': 'sidr',
  'عسل-الجرجير': 'jarjir',
  'عسل-الزعتر': 'zaatar',
  'عسل-الليمون': 'limon',
  'عسل-الكاليبتوس': 'eucalyptus',
  'عسل-الأعشاب': 'aachab',
  'عسل-الخروب': 'kharroub',
  'حبوب-اللقاح': 'bee-pollen',
  'نخالة-القاطونة': 'psyllium',
  'شيلاجيت': 'shilajit',
  'زيتون-طبيعي': 'zaytoun',
  'زيت-الزيتون': 'olive',
  'جرانولا': 'granola',
...
async fetch(request, env) {
  ...
  // Handle old Shopify URL patterns with 301 redirects
  const shopifyPrefixes = ['/ar/products/', '/products/', '/ar/pages/', '/pages/', '/ar/collections/', '/collections/', '/ar/cart', '/ar/blogs/', '/ar/'];
  for (const prefix of shopifyPrefixes) {
    if (path.startsWith(prefix)) {
      // Try to map /products/slug to new product
      if (prefix === '/ar/products/' || prefix === '/products/') {
        const oldSlug = path.replace(prefix, '').split('/')[0];
        const newSlug = SHOPIFY_PRODUCT_MAP[oldSlug] || SHOPIFY_PRODUCT_MAP[decodeURIComponent(oldSlug)];
        if (newSlug) {
          return redirect301(SITE + '/' + newSlug);
        }
      }
  ...
  if (assetsRes.status === 404 && !path.includes('.')) {
    const homeUrl = new URL(url);
    homeUrl.pathname = '/';
